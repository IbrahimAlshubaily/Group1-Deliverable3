...
public class Conference implements Serializable {
	...
	public boolean removeManuscript(Manuscript theManuscript) { 
		boolean returnValue = false;	
		if(theManuscript != null && myConferenceManuscripts.contains(theManuscript) ) {
			if (theManuscript.getReviewers().isEmpty()) {
				mySubmittedManuscripts.get(theManuscript.getSubmissionUser()).remove(theManuscript);
				myConferenceManuscripts.remove(theManuscript);
				myAuthors.remove(theManuscript.getSubmissionUser());
				removeCoAuthorSubmission(theManuscript);
				returnValue = true;
			}
		}
		return returnValue;
...
/**																														doc missing */
	private void removeCoAuthorSubmission(Manuscript theManuscript) {
		
		for(String theUserId : theManuscript.getAuthors()) {
			//if (mySubmittedManuscripts.containsKey(theUserId)) {
				mySubmittedManuscripts.get(theUserId).remove(theManuscript);
			//}		
		}
	}
	/**
